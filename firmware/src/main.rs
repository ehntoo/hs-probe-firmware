...
fn main() -> ! {
    ...
    let rcc = bsp::rcc::RCC::new(
        stm32ral::rcc::RCC::take().unwrap(),
        stm32ral::crs::CRS::take().unwrap(),
    );
    let nvic = bsp::nvic::NVIC::new(
        stm32ral::nvic::NVIC::take().unwrap(),
        stm32ral::scb::SCB::take().unwrap(),
    );
    let dma = bsp::dma::DMA::new(stm32ral::dma1::DMA1::take().unwrap());
    let gpioa = bsp::gpio::GPIO::new(stm32ral::gpio::GPIOA::take().unwrap());
    let gpiob = bsp::gpio::GPIO::new(stm32ral::gpio::GPIOB::take().unwrap());
    let spi = bsp::spi::SPI::new(stm32ral::spi::SPI1::take().unwrap());
    let mut uart = bsp::uart::UART::new(stm32ral::usart::USART2::take().unwrap(), &dma);
    let mut usb = bsp::usb::USB::new(stm32ral::usb::USB::take().unwrap());
