...
use super::{stat_nak, stat_stall, stat_valid, Endpoint, USBStackRequest};

use crate::app::{Mode, PinState, Request};
use crate::bsp::unique_id::get_hex_id;
...
impl ControlEndpoint {
    ...
    fn process_get_descriptor(
        ...
        match DescriptorType::try_from(descriptor_type) {
            Ok(DescriptorType::Device) => self.process_get_device_descriptor(usb, w_length),
        ...
        match VendorRequest::try_from(setup.bRequest) {
                ...
                match PinState::try_from(setup.wValue) {
                    Ok(ps) => {
                        self.pending_request =
                            Some(USBStackRequest::AppRequest(Request::SetCS(ps)));
                        self.transmit_ack(usb);
                    }
                    _ => {
                        self.stall(usb);
            ...
            Ok(VendorRequest::SetFPGA) => {
                match PinState::try_from(setup.wValue) {
                    Ok(ps) => {
                        self.pending_request =
                            Some(USBStackRequest::AppRequest(Request::SetFPGA(ps)));
                        self.transmit_ack(usb);
                    }
                    _ => {
                        self.stall(usb);
            ...
            Ok(VendorRequest::SetMode) => {
                match Mode::try_from(setup.wValue) {
                    Ok(mode) => {
                        self.pending_request =
                            Some(USBStackRequest::AppRequest(Request::SetMode(mode)));
                        self.transmit_ack(usb);
                    }
                    _ => {
                        self.stall(usb);
            ...
            Ok(VendorRequest::SetTPwr) => {
                match PinState::try_from(setup.wValue) {
                    Ok(ps) => {
                        self.pending_request =
                            Some(USBStackRequest::AppRequest(Request::SetTPwr(ps)));
                        self.transmit_ack(usb);
                    }
                    _ => {
                        self.stall(usb);
            ...
            Ok(VendorRequest::SetLED) => {
                match PinState::try_from(setup.wValue) {
                    Ok(ps) => {
                        self.pending_request =
                            Some(USBStackRequest::AppRequest(Request::SetLED(ps)));
                        self.transmit_ack(usb);
                    }
                    _ => {
                        self.stall(usb);
            ...
            Ok(VendorRequest::Bootload) => {
                self.pending_request =
                    Some(USBStackRequest::AppRequestAndDetach(Request::Bootload));
                self.transmit_ack(usb);
                None
            }
...
impl Endpoint for ControlEndpoint {
    ...
    fn process_transfer(&mut self, usb: &usb::Instance) -> Option<USBStackRequest> {
        let mut req = None;
        let (ctr_tx, ctr_rx, ep_type, ea) = read_reg!(usb, usb, EP0R, CTR_TX, CTR_RX, EP_TYPE, EA);
        if ctr_tx == 1 {
