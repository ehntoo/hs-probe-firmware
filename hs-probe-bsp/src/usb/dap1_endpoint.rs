...
use super::buffers::*;
use super::{stat_disabled, stat_nak, stat_stall, stat_valid, Endpoint, USBStackRequest};
...
impl Endpoint for DAP1Endpoint {
    ...
    fn reset_endpoint(&self, usb: &usb::Instance) {
        ...
        write_reg!(
            ...
            usb,
            EP2R,
            STAT_TX: stat_disabled(stat_tx),
            STAT_RX: stat_disabled(stat_rx)
        );
    }
    ...
    fn process_transfer(&mut self, usb: &usb::Instance) -> Option<USBStackRequest> {
        let mut req = None;
        let (ctr_tx, ctr_rx, ep_type, ea) = read_reg!(usb, usb, EP2R, CTR_TX, CTR_RX, EP_TYPE, EA);
        if ctr_tx == 1 {
            // Clear CTR_TX
            write_reg!(usb, usb, EP2R,
